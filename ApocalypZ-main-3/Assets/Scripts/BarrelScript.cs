            ...
            foreach (Collider collider in colliders)
            {
                ...
                if (rb != null && !alreadyAffectedRigidbodies.Contains(rb))
                {
                    ...
                    if (rb.TryGetComponent<PlayerScript>(out _))
                    {
                        ...
                        PlayerScript.instance.TakeDamage(explosionHitPoint);
                    }
                }
                // Break Wall with Barrel Explosion
                if (collider.TryGetComponent<BreakableWall>(out BreakableWall breakableWallScript))
                {
                    breakableWallScript.BreakWall();
                }
            }
